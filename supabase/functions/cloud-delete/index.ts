...
Deno.serve(async (req) => {
  ...
  const itemType = String(url.searchParams.get('type') ?? '').trim();
  const itemName = String(url.searchParams.get('name') || '').trim();

  if (!itemType || !itemName) return error('Tipo y nombre son requeridos.', 400);

  try {
    const jobId = await createJob(adminClient, {
      action: 'delete',
      path: requestedPath,
      payload: { type: itemType, name: itemName },
