const SITE_STATUS = "BETA Sitio en construcciÃƒÂ³n";
const SITE_VERSION = "V. 2.3.0";
...
function renderSubNav(module) {
  ...
  if (module === "media") {
    return [
      ...
      item("/media/#media-filters", "CategorÃƒÂ­as"),
      item(
        "/media/admin.html",
        "CMS",
        path.includes("/admin"),
  ...
  if (module === "media" && document.body.classList.contains("media-admin")) {
    return `
      ...
      <button class="hr-nav__action" id="logout-button" type="button">Cerrar sesiÃƒÂ³n</button>
    `;
  }
  ...
  if (document.body.classList.contains("db-body")) {
    return `
      ...
      <button class="hr-nav__account hr-nav__account--button" id="js-user-menu-toggle"
        ...
        aria-controls="js-user-menu js-sidebar" aria-label="Abrir menÃƒÂº">
        <span class="hr-nav__avatar" id="js-user-avatar" aria-hidden="true"></span>
        <span class="hr-nav__hello" id="js-user-display-name">ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â</span>
      </button>
      <nav class="db-user-menu" id="js-user-menu" aria-label="MenÃƒÂº de usuario" hidden>
        <ul class="db-user-menu__list" role="list">
          ...
          <li><button class="db-user-menu__item db-user-menu__item--danger" data-action="logout">Cerrar sesiÃƒÂ³n</button></li>
        </ul>
      </nav>
    `;
  }
...
function renderGlobalDrawer(activeModule) {
  ...
  const drawerSessionMarkup = isPortalDashboard
    ? `
          <div class="hr-global-drawer__guest hr-global-drawer__guest--portal">
            ...
            <button type="button" data-global-nav-action="logout">Cerrar sesiÃƒÂ³n</button>
          </div>
        `
    : `
  ...
  return `
    <button class="hr-global-drawer__backdrop" type="button"
      data-global-drawer-close aria-label="Cerrar menÃƒÂº" hidden></button>
    <aside class="hr-global-drawer" id="hr-global-drawer"
      aria-label="MenÃƒÂº principal" aria-hidden="true" hidden>
      <header class="hr-global-drawer__header">
        ...
        <button class="hr-global-drawer__close" type="button"
          data-global-drawer-close aria-label="Cerrar menÃƒÂº">Ãƒâ€”</button>
      </header>
      <p class="hr-global-drawer__label">Ecosistema</p>
      <nav class="hr-global-drawer__links" aria-label="NavegaciÃƒÂ³n mÃƒÂ³vil">
  ...
  list.innerHTML = items.length
    ? items.map((item) => `
        <li class="hr-notice hr-notice--${escapeNavText(item.type || "info")} hr-global-notifications__item${item.read ? " is-read" : ""}">
          ...
          <span class="hr-notice__message hr-global-notifications__message">${escapeNavText(item.message || "NotificaciÃƒÂ³n")}</span>
...
async function hydrateGlobalSession() {
  ...
  try {
    ...
    console.info("[HR] No fue posible hidratar la sesiÃƒÂ³n global:", error?.message || error);
  }
}
...
async function attachGlobalSessionSync() {
  ...
  try {
    ...
    console.info("[HR] No fue posible sincronizar la sesiÃƒÂ³n global:", error?.message || error);
  }
}
  ...
  target.innerHTML = `
    <header class="hr-nav" data-module="${module}" data-accent="${accent}">
      <div class="hr-nav__main">
        ...
        <nav class="hr-nav__links" aria-label="NavegaciÃƒÂ³n principal">
          ${ECOSYSTEM_LINKS.map(([key, href, label, adminOnly]) => `
        ...
        <button class="hr-nav__mobile-toggle" type="button" aria-label="Abrir menÃƒÂº"
          aria-controls="hr-global-drawer" aria-expanded="false">
          <span></span><span></span><span></span>
        </button>
      </div>
      ${subnav ? `<nav class="hr-nav__sub" aria-label="NavegaciÃƒÂ³n contextual">${subnav}</nav>` : ""}
    </header>
    ...
    <aside class="hr-global-notifications" id="hr-global-notifications"
      ...
      <header>
        ...
        <button type="button" data-hr-notifications-close aria-label="Cerrar notificaciones">Ãƒâ€”</button>
      </header>
      <ul data-hr-notifications-list>
        <li class="hr-global-notifications__empty">Cargando notificacionesÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦</li>
      </ul>
    </aside>
